...
load realdata_airplane.mat
X=test;

Fc=9.5e9;   %the center freqnency of the radar
B=1e9;      %the bandwidth of the pulse
T=200e-6;   %the time duration of the pulse
k=B/T;      %the frequency slope of the chirp signal
fs=10e6;    %the sampling frequency
Ts=1/fs;    %the sampling interval
PRF=1000;   %the pulse repeat frequency
PRI=1/PRF;  %the pulse repeat interval
dr=3e8/2/B;
r=size(X,2)*dr;
r_label=0:r/512:r-r/512;

sig=(X(1+256:4:256+256,:));
RPP=sig;
...
[ret,E]=FMEPC(fmepc);
ret=(abs(flipud(ret)));
eval(['save real_fmepc',' ret r_label']) ;
...
eval(['save real_pga',' ret r_label']) ;
...
eval(['save real_admm',' ret r_label']) ;
imagesc(r_label,1:size(ret,1),ret)
...
eval(['save real_adn',' ret r_label']) ;
...
ret=(py_data);
ret=((abs(ret)/max(max(abs(ret)))));
eval(['save real_deep',' ret r_label']) ;
imagesc(r_label,1:size(ret,1),(ret));
