...
class ADMMCSNetLayer2(nn.Module):
    def __init__(
        self,
        mask,
        args,
        ...
        """
        super(ADMMCSNetLayer2, self).__init__()

        self.rho = nn.Parameter(torch.tensor([0.2]), requires_grad=True)
...
class ReconstructionUpdateLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # number =torch.matmul(torch.matmul(torch.fft.fft2(minus,self.data_size),E)-input,E.conj().transpose(-1,-2))
        tmp=torch.fft.ifftn(minus,self.data_size[1],dim=-1)
        tmp=torch.fft.fftn(tmp,self.data_size[0],dim=-2)
        number=torch.matmul(torch.matmul(tmp,E)-input,E.conj().transpose(-1,-2))
        denom = torch.add(mask, self.rho)
        a = 1e-6
        value = torch.full(denom.size(), a)
        denom = torch.where(denom == 0, value, denom)
        ...
        # orig_output3 = minus-torch.fft.ifft2(orig_output2,self.fr_size)
        orig_output3_tmp = torch.fft.ifftn(orig_output2,self.fr_size[0],dim=-2)
        orig_output3=torch.fft.fftn(orig_output3_tmp,self.fr_size[1],dim=-1)
        x['re_mid_output'] = minus-orig_output3
        return x
...
class ReconstructionFinalLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        orig_output3_tmp = torch.fft.ifftn(orig_output2,self.fr_size[0],dim=-2)
        orig_output3=torch.fft.fftn(orig_output3_tmp,self.fr_size[1],dim=-1)
        x['re_final_output'] = minus-orig_output3
