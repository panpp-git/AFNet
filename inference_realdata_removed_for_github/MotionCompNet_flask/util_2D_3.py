...
def load(fn, module_type, device = torch.device('cuda')):
    ...
    if device == torch.device('cpu'):
        args.use_cuda = False
    if module_type == 'pre':
        model = complexModule.set_pre_module(args)
    else:
...
def set_optim(args, module, module_type):
    if module_type == 'fr':
        ...
        # optimizer = torch.optim.RMSprop(module.parameters(), lr=args.lr_fr, alpha=0.9, weight_decay=0.003)
    elif module_type == 'fc':
        optimizer = torch.optim.Adam(module.parameters(), lr=args.lr_fc,weight_decay=0.02)
    ...
    elif module_type=='dncnn':
        optimizer = torch.optim.Adam(module.parameters(), lr=args.n_lr)
    else:
