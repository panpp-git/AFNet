...
from complexFunctions import complex_relu, complex_max_pool2d, complex_avg_pool2d,complex_relu2,complex_relu3
...
def apply_complex2(fr, fi, input):
    input_real,input_imag=torch.chunk(input,2,1)
    ret_real=(fr(input_real) - fi(input_imag))
    ret_imag=fr(input_imag) + fi(input_real)
...
class ComplexReLU2(Module):

    def forward(self, input):
        return complex_relu2(input)

class ComplexReLU3(Module):

    def forward(self, input,input_abs):
        return complex_relu3(input,input_abs)
...
class _ComplexBatchNorm(Module):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
        ...
        if self.track_running_stats:
            ...
            self.register_buffer('running_mean_r', torch.zeros(num_features, dtype=torch.float32))
            self.register_buffer('running_mean_i', torch.zeros(num_features, dtype=torch.float32))
...
class ComplexBatchNorm2d_2(_ComplexBatchNorm):

    def forward(self, input):
        input_r,input_i=torch.chunk(input,2,1)
        # input=input_r.type(torch.complex64)+1j*input_i.type(torch.complex64)
        exponential_average_factor = 0.0
        ...
        input=torch.cat((input_r,input_i),1)
        return input
