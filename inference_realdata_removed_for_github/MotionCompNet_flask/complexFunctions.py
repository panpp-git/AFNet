...
def complex_relu(input):
    absv=relu(input.abs())
    anglev=torch.atan2(input.imag,input.real)
    return (absv*torch.cos(anglev)).type(torch.complex64)+1j* (absv*torch.sin(anglev)).type(torch.complex64)


def complex_relu2(input):
    input_r,input_i=torch.chunk(input,2,1)
    input_abs=torch.sqrt(torch.pow(input_r,2)+torch.pow(input_i,2))
    absv = relu(input_abs)
    anglev = torch.atan2(input_i, input_r)
    ret_r=(absv * torch.cos(anglev))
    ret_i=(absv * torch.sin(anglev))
    ret=torch.cat((ret_r,ret_i),1)
    return ret

def complex_relu3(input,input_abs):
    input_r,input_i=torch.chunk(input,2,1)
    absv = input_abs
    anglev = torch.atan2(input_i, input_r)
    ret_r=(absv * torch.cos(anglev))
    ret_i=-1*(absv * torch.sin(anglev))
    ret=torch.cat((ret_r,ret_i),1)
    return ret
